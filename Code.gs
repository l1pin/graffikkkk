function getDataBySql(
  strSQL = "SELECT * FROM `ads_collection` WHERE `source` = 'TikTok'"
) {
  try {
...
function openAnalyticsWebApp() {
  try {
    ...
    const webAppUrl =
      "https://script.google.com/macros/s/AKfycbxExFx26-bi-mifd_75BXg6euym3BpYfZidAJ4tFJdPVLrReTXzmjk6acWd2RMb8DPCWQ/exec";
...
function buildChartForArticle(article, periodStart, periodEnd) {
  ...
  try {
    ...
    function formatValueByRow(value, rowIndex) {
      // Для названий рекламы (индекс 17) и URL (18) возвращаем как строку
      if (rowIndex === 17 || rowIndex === 18) {
...
`;
    ...
    allRows.forEach((row) => {
      ...
      if (hasMetrics && (campaignName || groupId)) {
        console.log(
          `📘 Processing TikTok metrics for buyer: ${buyerInfo.buyer}, group: ${groupName}, date: ${dateStr}`
        );
    ...
    function processSegment(
      ...
      console.log(
        `✅ Segment ${segmentName} has data from ${
          segmentMinDate.toISOString().split("T")[0]
    ...
    Object.keys(buyerGroupsData).forEach((buyer) => {
      console.log(
        `  👤 ${buyer}: ${
          Object.keys(buyerGroupsData[buyer].groups).length
        } groups`
      );
    });
